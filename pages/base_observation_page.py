...
class BaseObservationPage(QWidget):
    ...
    def record_response(self, category, response, value=None):
        ...
        self.responses.append((t, category, response, value))
        print(f"Recorded {category}: {response} (value: {value}) at {t:.1f}s")
    ...
    def stop_observation(self):
        ...
        if path:
            with open(path, "w", newline="") as f:
                ...
                protocol_name = current_config.get('name', 'Unknown')
                start_datetime = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(self.start_time))
                
                writer.writerow([f"# Protocol: {protocol_name}"])
                writer.writerow([f"# Observation Started: {start_datetime}"])
                writer.writerow([f"# Total Duration: {time.time() - self.start_time:.1f} seconds"])
                writer.writerow([f"# Generated by: REFLECT v1.0"])
